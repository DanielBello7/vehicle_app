...
class DatabaseAccess implements DatabaseAccessType {
  ...
  public CreateUser = async (data: NewUserType): Promise<DatabaseResponse> => {
    ...
    const [confirmation]: any = await this.conn.promise().query(`SELECT * FROM users WHERE email = ?`, [data.email]);
  ...
  public GetUser = async (email: string): Promise<DatabaseResponse> => {
    const [response]: any = await this.conn.promise().query('SELECT * FROM users WHERE email = ?', [email]);
  ...
  public ConfirmRegister = async (id: string): Promise<DatabaseResponse> => {
    const [request]: any = await this.conn.promise().query('SELECT * FROM registered WHERE _id = ?', [id]);
    if (request.length <= 0) return {msg: 'not found', result: {found: false}, success: true}
  ...
  public GetRegisterData = async (filter: 'email' | '_id' | 'license', data: string): Promise<DatabaseResponse> => {
    const [request] = await this.conn.promise().query(`SELECT * from registered WHERE ${filter} = ?`, [data]);
  ...
  public CreateNewRegister = async (data: NewRegisterType): Promise<DatabaseResponse> => {
    ...
    const [confirm]: any = await this.conn.promise().query('SELECT * FROM registered WHERE license = ?', [license]);

    if (confirm.length > 0) return {msg: 'license already registered', result: confirm, success: true}
